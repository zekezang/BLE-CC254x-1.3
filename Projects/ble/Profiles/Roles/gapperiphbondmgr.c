...
static uint8 gapBond_KeyDistList = (GAPBOND_KEYDIST_SENCKEY // sEncKey enabled, to send the encryption key
...
static uint8 gapBondMgrAddBond(gapBondRec_t *pBondRec, gapBondLTK_t *pLocalLTK, gapBondLTK_t *pDevLTK, uint8 *pIRK, uint8 *pSRK,
		uint32 signCounter);
...
bStatus_t GAPBondMgr_SetParameter(uint16 param, uint8 len, void *pValue) {
	...
	case GAPBOND_DEFAULT_PASSCODE:
		if ((len == sizeof(uint32)) && (*((uint32*) pValue) <= GAP_PASSCODE_MAX)) {
	...
	case GAPBOND_KEYSIZE:
		if ((len == sizeof(uint8)) && ((*((uint8*) pValue) >= MIN_ENC_KEYSIZE) && (*((uint8*) pValue) <= MAX_ENC_KEYSIZE))) {
	...
	if (idx < GAP_BONDINGS_MAX) {
		...
		if (osal_snv_read(localLTKNvID(idx), sizeof(smSecurityInfo_t), &localLTK) == SUCCESS) {
			if ((localLTK.keySize >= MIN_ENC_KEYSIZE) && (localLTK.keySize <= MAX_ENC_KEYSIZE)) {
				...
				VOID GAP_Bond(connHandle, ((stateFlags & GAP_BONDED_STATE_AUTHENTICATED) ? TRUE : FALSE), &localLTK);
			}
		}
		...
		if (osal_snv_read(devCSRKNvID(idx), KEYLEN, signingInfo.srk) == SUCCESS) {
			if (osal_isbufset(signingInfo.srk, 0xFF, KEYLEN) == FALSE) {
				...
				VOID GAP_Signable(connHandle, ((stateFlags & GAP_BONDED_STATE_AUTHENTICATED) ? TRUE : FALSE), &signingInfo);
			}
		}
...
void gapBondMgr_ProcessGAPMsg(gapEventHdr_t *pMsg) {
	...
	case GAP_AUTHENTICATION_COMPLETE_EVENT: {
		...
		if ((pPkt->hdr.status == SUCCESS) && (pPkt->authState & SM_AUTH_STATE_BONDING)) {
			...
			VOID gapBondMgrAddBond(&bondRec, (gapBondLTK_t *) pPkt->pSecurityInfo, (gapBondLTK_t *) pPkt->pDevSecInfo,
