...
// Value to write
static uint32 simpleBLECharVal = 0;
...
// GAP Role Callbacks
static const gapCentralRoleCB_t simpleBLERoleCB = { simpleBLECentralRssiCB, // RSSI callback
		simpleBLECentralEventCB // Event callback
		};

// Bond Manager Callbacks
static const gapBondCBs_t simpleBLEBondCB = { simpleBLECentralPasscodeCB, simpleBLECentralPairStateCB };
...
static void simpleBLECentral_HandleKeys(uint8 shift, uint8 keys) {
	...
	if (keys & HAL_KEY_UP) {
		...
		} else if (simpleBLEState == BLE_STATE_CONNECTED && simpleBLECharHdl != 0 && simpleBLEProcedureInProgress == FALSE) {

			HalLcdWriteString("BLE master zekezang", HAL_LCD_LINE_1);
			if (simpleBLEState == BLE_STATE_CONNECTED) {
				if (!simpleBLERssi) {
					simpleBLERssi = TRUE;
	...
	if (keys & HAL_KEY_LEFT) {
		...
		if (!simpleBLEScanning && simpleBLEScanRes > 0) {
			...
			LCD_WRITE_STRING_VALUE( "Device", simpleBLEScanIdx + 1, 10, HAL_LCD_LINE_1);
 ...
 */
static void simpleBLECentralRssiCB(uint16 connHandle, int8 rssi) {
	LCD_WRITE_STRING_VALUE( "RSSI -dB:", (uint8) (-rssi), 10, HAL_LCD_LINE_4);
}
...
static void simpleBLECentralEventCB(gapCentralRoleEvent_t *pEvent) {
	...
	case GAP_DEVICE_DISCOVERY_EVENT: {
		...
		LCD_WRITE_STRING_VALUE( "Devices Found", simpleBLEScanRes, 10, HAL_LCD_LINE_1);
		if (simpleBLEScanRes > 0) {
